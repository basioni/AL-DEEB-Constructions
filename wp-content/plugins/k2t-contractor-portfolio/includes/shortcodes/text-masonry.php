<?php
	...
	$portfolio_sub_title = ( function_exists( 'get_field' ) ) ? get_field( 'portfolio_sub_title', get_the_ID() ) : ''; $portfolio_sub_title = empty( $portfolio_sub_title ) ? '' : $portfolio_sub_title;
...
<article class="<?php echo esc_attr( $post_classes ) ;?>"><div class="article-inner">
	...
	<div class="post-text">
		<header class="title-meta">
			<h3 class="title">
                <a href="<?php esc_url ( the_permalink() );?>" title="<?php echo esc_attr( $title );?>"><?php echo $title;?></a>
            </h3>
		</header><!-- .title-meta -->
		<?php if ( $portfolio_display_info == 'excerpt' ):?>
		<div class="excerpt">
			<?php the_excerpt();?>
		</div><!-- .excerpt -->
		<?php elseif ( $portfolio_display_info == 'meta' ):?>
		<div class="excerpt">
		<?php
			if ( count( $categories ) > 0 && is_array( $categories ) ) {
                $i = 0;
