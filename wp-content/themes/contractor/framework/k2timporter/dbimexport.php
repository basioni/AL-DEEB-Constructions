...
class dbimexport
{
    ...
    private function setconnection()
    {
        ...
        $this->link = mysqli_connect($this->db_config['host'], $this->db_config['user'], $this->db_config['password'],$this->db_config['database']);
    ...
    public function export()
    {
        ...
        $neednt_array = array( 'wp_options' , 'wp_usermeta' , 'wp_users' );
        while ( $tableRow = mysqli_fetch_row ( $tableQuery ) )
        {

            if( !in_array( $tableRow[ 0 ] , $neednt_array ) ){
                //Table Node
        ...
        while ( $tableRow = mysqli_fetch_row ( $tableQuery ) )
        {
            ...
            if( !in_array( $tableRow[ 0 ] , $neednt_array ) ){
                ...
                $schema = Array();
                while ( $row = mysqli_fetch_assoc ( $descQuery ) )
                {
