...
<?php
		...
		function process_posts() {
			...
			foreach ( $this->posts as $post ) {
				/* Update Posts Value */
				$i_proccess = $i_proccess + 1;
				$cache_theme_posts_install = get_template_directory() . '/framework/k2timporter/tmp_backup/cache_proccess/theme_posts.txt';
				file_put_contents($cache_theme_posts_install, $post['post_type'] . " : " . $post['post_name']);
				/* Update Posts Process */
				$cache_theme_posts_install = get_template_directory() . '/framework/k2timporter/tmp_backup/cache_proccess/theme_progess.txt';
				file_put_contents($cache_theme_posts_install, count($this->posts) . "|" . (string)$i_proccess);
				
				//setcookie("theme_posts", $post['post_type'] . " : " . $post['post_name'] , time() + (86400 * 30), "/");
				//setcookie("theme_progess", count($this->posts) . "|" . (string)$i_proccess , time() + (86400 * 30), "/");	
				//k2t_options("theme_posts", $post['post_type'] . " : " . $post['post_name'] );
				//k2t_options("theme_progess", count($this->posts) . "|" . (string)$i_proccess , time() + (86400 * 30), "/");
