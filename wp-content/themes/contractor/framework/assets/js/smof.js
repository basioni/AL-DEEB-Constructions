...
jQuery(document).ready(function($){
	...
	*/
	...
	function loop_active_plugin(current_plugin,pl){		
		...
		jQuery.get(home_url + '/wp-admin/themes.php?page=tgmpa-install-plugins&plugin=' + pl[current_plugin]["slug"] + '&plugin_name=' + pl[current_plugin]["name"].replace(" ", "%20")  + plugin_source_temp + '&tgmpa-install=install-plugin&_wpnonce=' + pl[current_plugin]["install_nonce"], function( data ) {
			...
			jQuery.get(home_url + '/wp-admin/themes.php?page=tgmpa-install-plugins&plugin=' + pl[current_plugin]["slug"] + '&plugin_name=' + pl[current_plugin]["name"].replace(" ", "%20")  + plugin_source_temp + '&tgmpa-activate=activate-plugin&tgmpa-activate-nonce=' + pl[current_plugin]["activate_nonce"], function( data ) {
				current_plugin++;
				jQuery(".process_percent_container .process_percent").attr('current_percent','10').css("width","10%").html("<span>Installing... 10%</span>");
					...
					jQuery.post(ajaxurl, data1, function(response){
						...
						*/
						jQuery.post(ajaxurl, data2, function(response){
							...
							/* Clear Cache */
							jQuery.get( home_url + "/wp-admin/upgrade.php?step=1&backto=%2Fcontractor%2Fwp-admin%2Fthemes.php%3Fpage%3Doptionsframework",function(){});
							/* reset permalink */
